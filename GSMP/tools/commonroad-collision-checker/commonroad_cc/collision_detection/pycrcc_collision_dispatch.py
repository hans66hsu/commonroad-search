...
from commonroad_cc.collision_detection.minkowski_sum import minkowski_sum_circle
import commonroad_cc.collision_detection.scenario


def _create_collision_object_dict():
    collision_object_func = {}
    if 'commonroad_cc.collision_detection.scenario' in sys.modules.keys():
        collision_object_func.update(
            commonroad_cc.collision_detection.scenario.collision_object_func_dict)
...
def _create_collision_checker_dict():
    ...
    if 'commonroad_cc.collision_detection.scenario' in sys.modules.keys():
        collision_checker_func.update(
            commonroad_cc.collision_detection.scenario.collision_checker_func_dict)
