...
template <typename T> std::ofstream TestFailureLogger<T>::out_file;
template <typename T> std::mutex TestFailureLogger<T>::file_mutex;
template <typename T> int TestFailureLogger<T>::num_copies = 0;

template <typename T> TestFailureLogger<T>::TestFailureLogger(void) {
...
template <>
const char *
TestFailureLogger<TestFailureLoggerNarrowphase>::get_filename(void) {
