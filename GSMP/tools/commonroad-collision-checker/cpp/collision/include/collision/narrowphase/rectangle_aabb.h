...
public:
  ...
  RectangleAABB(double _rx, double _ry,
    ...
    max_ = center_ + r_;
    segments_.push_back(
        LineSegment(min_, Eigen::Vector2d(min_.x() + 2 * _rx, min_.y())));
    segments_.push_back(
        LineSegment(min_, Eigen::Vector2d(min_.x(), min_.y() + 2 * _ry)));
    segments_.push_back(
        LineSegment(max_, Eigen::Vector2d(max_.x(), max_.y() - 2 * _ry)));
    segments_.push_back(
        LineSegment(max_, Eigen::Vector2d(max_.x() - 2 * _rx, max_.y())));
  }
