...
namespace solverFCL {
CollisionObject *
getParentPointerFromFclObj(fcl::CollisionObject<FCL_PRECISION> *fcl_obj);
...
public:
  CollisionRequestDataMultipleObstacles(
      ...
      int max_obstacles)
      : m_obstacles(ret_obstacles), m_parent_map(parent_map),
  ...
  void
  setSubjectEntitySize(fcl::BroadPhaseCollisionManager<FCL_PRECISION> *mngr) {
...
public:
  CollisionRequestDataWindowQuery(
      ...
      const std::unordered_map<const collision::CollisionObject *,
                               std::list<CollisionObjectConstPtr>> &parent_map)
      : m_subject_object(subject_object), m_result(result),
...
template <typename S> struct CollisionData {
...
template <typename S> struct CollisionDataEx : public CollisionData<S> {
...
template <typename S> struct CollisionDataOverlap : public CollisionDataEx<S> {
...
bool defaultCollisionFunctionListOfObstacles(fcl::CollisionObject<S> *o1,
  ...
  if (!(cdata->self_reqData.testPair(pair_obj)))
    return false;
