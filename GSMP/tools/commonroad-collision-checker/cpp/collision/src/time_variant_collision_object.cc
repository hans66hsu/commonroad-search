...
TimeVariantCollisionObject::getObstacleAtTime(int time_idx) const {
  if (time_idx < time_start_idx_ || time_idx > time_end_idx_)
    return nullptr;
...
exportObject(const collision::TimeVariantCollisionObject &);
}

serialize::ICollisionObjectExport *
TimeVariantCollisionObject::exportThis(void) const {
