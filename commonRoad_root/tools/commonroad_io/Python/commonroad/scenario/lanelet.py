...
class Lanelet:
    ...
    @classmethod
    def all_lanelets_by_merging_successors_from_lanelet(cls, lanelet, network):
        """
        ...
        # Calculate all paths to leaf nodes
        for leaf in leafs:
        ...
        # Visuallization of Graph
        # import matplotlib.pyplot as plt
        # plt.figure()
        # nx.draw(Net, with_labels=True)
        # plt.show() # display

        # Create merged lanelets
...
class LaneletNetwork:
    ...
    def create_from_lanelet_list(cls, lanelets: list):
        ...
        assert isinstance(lanelets, list) and all(isinstance(l, Lanelet) for l in
                                                  lanelets), '<LaneletNetwork/create_from_lanelet_list>: Provided list of lanelets is not valid! lanelets = {}'.format(
            lanelets)
