...
class CommonRoadFileWriter:
    def __init__(self, scenario: Scenario, planning_problem_set: PlanningProblemSet, author:str, affiliation: str,
                 source: str, tags: str, decimal_precision: int=8):
        """
...
class StateXMLNode:
    ...
    def create_goal_state_node(cls, state: State, goal_lanelet_ids: List[int]) -> et.Element:
        ...
        if hasattr(state, 'velocity'):
            ...
            velocity = cls._write_value_exact_or_interval(
        ...
        if hasattr(state, 'yaw_rate'):
            ...
            yaw_rate = cls._write_value_exact_or_interval(
    ...
    @classmethod
    def _write_value_exact_or_interval(cls, node: et.Element, var: Union[Interval, float, int]):
        """
    ...
    def create_state_node(cls, state: State, state_node: et.Element, time_step: int) -> et.Element:
        ...
        if hasattr(state, 'orientation'):
            orientation = et.Element('orientation')
            orientation = cls._write_value_exact_or_interval(orientation, state.orientation)
            state_node.append(orientation)
        ...
        if hasattr(state, 'acceleration'):
            acceleration = et.Element('acceleration')
            acceleration = cls._write_value_exact_or_interval(acceleration, state.acceleration)
            state_node.append(acceleration)
        if hasattr(state, 'yaw_rate'):
            yaw_rate = et.Element('yawRate')
        ...
        if hasattr(state, 'slip_angle'):
            slip_angle = et.Element('slipAngle')
            slip_angle = cls._write_value_exact_or_interval(slip_angle, state.slip_angle)
            state_node.append(slip_angle)
        return state_node
