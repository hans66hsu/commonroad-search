        ...
        if hasattr(self, 'position'):
            ...
            if isinstance(self.position, ValidTypes.ARRAY):
                transformed_state.position = commonroad.geometry.transform.translate_rotate(
                    np.array([self.position]), translation, angle)[0]
            # uncertain position:
...
class Trajectory:
    ...
    def initial_time_step(self, initial_time_step):
        assert isinstance(initial_time_step, int), '<Trajectory/initial_time_step>: argument initial_time_step of ' \
    ...
    def state_list(self, state_list):
        ...
        assert self.initial_time_step == state_list[0].time_step if hasattr(state_list[0], 'time_step') else True, \
    ...
    def state_at_time_step(self, time_step: int) -> Union[State, None]:
        ...
        """
        assert isinstance(time_step, int), '<Trajectory/state_at_time_step>: argument time_step of ' \
                                           'wrong type. Expected type: %s. Got type: %s.' \
                                          % (int, type(time_step))
        state = None
    ...
    def translate_rotate(self, translation: np.ndarray, angle: float):
        ...
        :param angle: rotation angle in radian (counter-clockwise)
        """
        assert is_real_number_vector(translation, 2), '<Trajectory/translate_rotate>: argument translation is not ' \
                                                      'a vector of real numbers of length 2.'
        assert is_real_number(angle), '<Trajectory/translate_rotate>: argument angle must be a scalar. ' \
                                      'angle = %s' % angle
        assert is_valid_orientation(angle), '<Trajectory/translate_rotate>: argument angle must be within the ' \
                                            'interval [-2pi,2pi]. angle = %s' % angle
