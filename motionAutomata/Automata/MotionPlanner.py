...
class MotionPlanner:
    ...
    def lanelets_of_position(self, lanelets: List[int], state: State, diff:float = math.pi/5) -> List[Lanelet]: 
        """
    ...
    def is_goal_in_lane(self, lanelet_id: int, traversed_lanelets = None) -> bool:
        ...
        :param traversed_lanelets: helper variable which stores potential path (a list of lanelet id) to goal lanelet. Default value is None. 
    ...
    def calc_heuristic_lanelet(self, path: List[State]) -> Tuple[float, list, list]: 
        """
    ...
    def translate_primitive_to_current_state(self, primitive: MotionPrimitive, currentPath: List[State]) -> List[State]:
        ...
        :param currentPath: the path whose last state is the goal state for the translation
        """
    ...
    def search_alg(self, startSuccessor: List[MotionPrimitive], maxTreeDepth: int, status: dict) -> Tuple[List[State], List[MotionPrimitive]]:
        """
        ...
        :param status: helper values including 'cost_so_far', 'time_step', 'orientation', 'velocity', which are used for the visualization in the jupyter notebook
        ...
        # update status for jupyter notebook visualization
        stat = {}
...
def distance(pos1: np.ndarray, pos2: np.ndarray, type=0) -> float:
    """
